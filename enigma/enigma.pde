...
int stepState = -1; // tracks what step it's on
...
void draw() {
  ...
  if (stepping) {
    if (counter < curMessage.length()*stepNum) {
      ...
      // EXTREMELY SPAGHETTI - MODIFY IF WE HAVE TIME - PRINTS ROTORS IF STEPPING NOT DONE
      // print rotors
    // for reference: stepMod = enigmaPackets[counter/stepNum][stepState][0].charAt(0);
    // for reference2: rotorVisuals = rotorSplitterCustom(rotor3, Integer.parseInt(enigmaPackets[counter/stepNum][stepState][1]));
    String[] rotorVisuals = rotorSplitter(rotor3);
    if (modified.length() > 0) {
      rotorVisuals = rotorSplitterCustom(rotor3, Integer.parseInt(enigmaPackets[counter/stepNum][stepState][1]));
    ...
    if (modified.length() > 0) {
      rotorVisuals = rotorSplitterCustom(rotor2, Integer.parseInt(enigmaPackets[counter/stepNum][stepState][2]));
    ...
    if (modified.length() > 0) {
      rotorVisuals = rotorSplitterCustom(rotor1, Integer.parseInt(enigmaPackets[counter/stepNum][stepState][3]));
      ...
      // EXTREMELY SPAGHETTI - MODIFY IF WE HAVE TIME - PRINTS ROTORS IF STEPPING DONE
      String[] rotorVisuals = rotorSplitterCustom(rotor3, Integer.parseInt(enigmaPackets[modified.length()-1][stepNum-1][1]));
      rotorPrinter(rotorVisuals, 420, modified.charAt(modified.length()-1));
      rotorVisuals = rotorSplitterCustom(rotor2, Integer.parseInt(enigmaPackets[modified.length()-1][stepNum-1][2]));
      rotorPrinter(rotorVisuals, 520, modified.charAt(modified.length()-1));
      rotorVisuals = rotorSplitterCustom(rotor1, Integer.parseInt(enigmaPackets[modified.length()-1][stepNum-1][3]));
      rotorPrinter(rotorVisuals, 620, modified.charAt(modified.length()-1));
    }
